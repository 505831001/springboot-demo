 ...
 *     二、解决方案
 *     1.Spring Session提供了用于管理用户会话信息的API和实现。
 *     2.Spring Session使得支持集群会话变得微不足道，而不依赖于特定于应用程序容器的解决方案。它还提供透明集成：
 *         1.HttpSession - 允许以应用程序容器（即Tomcat）中立的方式替换HttpSession，支持在头文件中提供会话ID以使用RESTFul API。
 ...
 *     5.从官方介绍中可以了解到Spring Session是用来解决，集群下实现全局Session的一个框架实现。全局Session也可以理解成分布式Session。
...
public class DemoSessionServerApplication {
    public static void main(String[] args) {
        ConfigurableApplicationContext context = SpringApplication.run(DemoSessionServerApplication.class, args);
