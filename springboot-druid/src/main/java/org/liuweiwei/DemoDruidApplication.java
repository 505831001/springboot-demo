...
public class DemoDruidApplication {
    ...
    /*类，可以用来从Java主方法引导和启动Spring应用程序。默认情况下，class将执行以下步骤来引导应用程序。*/
    private static SpringApplication application;
    /*大部分(如果不是全部)应用程序上下文都要实现的SPI接口。除了{ApplicationContext}接口中的应用程序上下文客户端方法之外，还提供了配置应用程序上下文的工具。*/
    private static ConfigurableApplicationContext applicationContext;
    /**大多数(如果不是全部){Environment}类型都要实现的配置接口。提供用于设置活动和默认概要文件以及操作底层属性源的工具。允许客户端通过{ConfigurablePropertyResolver}超接口设置和验证所需的属性，自定义转换服务和更多。*/
